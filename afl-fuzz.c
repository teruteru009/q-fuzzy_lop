...
double updateq(int s,int snext ,int a,double qvalue[][ACTIONNO],int stop_flag){
	double qv;
	if(stop_flag==1)
...
int nexts(int s,int a,int stage_val){
	int stage_snum=1;
        int num_states=0;
        for(int i=1;i<stage_val;i++){
                 stage_snum*=ACTIONNO;
                 num_states+=stage_snum;
        }
        int  next_num_states=stage_snum*ACTIONNO+num_states;
        return ACTIONNO*(s-num_states)+next_num_states-(ACTIONNO-1-a);
}
...
static u8 fuzz_one(char** argv) {
  ff=fopen("../afl-2.52b-q1.0/test.txt","w+");
 ...
 i if (queue_cur->depth > 1) return 1;
...
havoc_stage:
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
   ...
   // u32 use_stacking = 1 << (1 + UR(HAVOC_STACK_POW2));
    ...
    for (i = 0; i < LEVEL; i++) {//**
      if(temp_len <4) break;
      a=selecta(s,qvalue);
      snext=nexts(s,a,stage_cur_val-1);
      switch (a) {
      ...
      qvalue[s][a]=updateq(s,snext,a,qvalue,stop_flag);
      fprintf(ff,"s:%d,snext:%d,a:%d,stop_flag%d\n",s,snext,a,stop_flag);
      s=snext;
      stage_subcount++;
      stage_cur_val++;
      if(stop_flag==1){
  ...
  if (!splice_cycle) {
    ...
    stage_cycles[STAGE_HAVOC] += stage_subcount;//**
  } else {
    stage_finds[STAGE_SPLICE]  += new_hit_cnt - orig_hit_cnt;
    stage_cycles[STAGE_SPLICE] += stage_subcount;//**
  }
